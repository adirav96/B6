...
export default function InterviewSession() {
  ...
  if (problemsLoading || (!problem && problems.length === 0)) {
    return (
  ...
  const tabClass = (tab) =>
    `flex-1 py-3 text-sm font-medium transition-colors ${activeTab === tab
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)] w-full overflow-x-hidden">
      ...
      {/* Simulation bar */}
      {simSession && (
        <div className="flex items-center justify-between bg-gray-900 border-b border-gray-700 px-4 py-2 shrink-0 gap-3">
          {/* Timer */}
          <div className={`flex items-center gap-2 font-mono font-bold text-sm tabular-nums shrink-0 ${simUrgent ? 'text-red-400 animate-pulse' : 'text-primary'}`}>
          ...
          <div className="flex gap-1.5 overflow-x-auto">
            {simSession.problemIds.map((id, idx) => {
              const p = problems.find((problemItem) => problemItem.id === id);
              ...
              return (
                <button
                  ...
                  className={`px-3 py-1 rounded-lg text-xs font-medium whitespace-nowrap transition-colors ${isCurrent
          ...
          <button
            ...
            className="text-gray-400 hover:text-white text-xs flex items-center gap-1 shrink-0 transition-colors"
          >
      ...
      <div className="flex flex-col lg:flex-row flex-1 min-h-0 min-w-0">
        <div className="w-full lg:w-2/5 flex flex-col border-b lg:border-b-0 lg:border-l border-purple-200 dark:border-gray-700 bg-purple-50 dark:bg-gray-800 min-h-0 min-w-0 h-[45vh] lg:h-auto lg:shrink-0">
