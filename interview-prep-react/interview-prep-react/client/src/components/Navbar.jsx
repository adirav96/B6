...
export default function Navbar() {
  ...
  const linkClass = ({ isActive }) =>
    `px-4 py-5 text-sm font-medium cursor-pointer transition-colors ${
      isActive ? 'text-primary border-b-[3px] border-primary' : 'text-gray-600 hover:text-primary'
    }`;

  const mobileLinkClass = ({ isActive }) =>
    `block px-4 py-3 text-sm font-medium transition-colors rounded-lg ${
      isActive ? 'text-primary bg-indigo-50' : 'text-gray-600 hover:text-primary hover:bg-gray-50'
    }`;
        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link to="/profile" className="flex items-center gap-2">
              <img src={`https://ui-avatars.com/api/?name=${encodeURIComponent(user?.name || '')}&background=6366f1&color=fff&size=32`} className="w-8 h-8 rounded-full" alt="avatar" />
